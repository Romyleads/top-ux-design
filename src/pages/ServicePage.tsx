      ...
      <div className="max-w-[1000px] mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-10">
          ...
          <div className="flex flex-col">
            ...
            {/* Tier picker */}
            <div className="rounded-xl p-1 mb-4" style={{ background: 'hsl(220 14% 96%)' }}>
              <div className="grid grid-cols-3 gap-0.5">
                {service.tiers.map((tierItem, i) => (
                  <button
                    key={i}
                    onClick={() => setActiveTier(i)}
                    className={`flex flex-col items-center py-3 px-2 rounded-lg cursor-pointer transition-all duration-200 ${
                  ...
                  >
                    <span className={`text-[11px] font-semibold leading-none mb-1 ${i === activeTier ? "text-primary font-bold" : "text-t4"}`}>
                      {tierItem.name}
                    </span>
                    <span className={`text-[17px] font-extrabold leading-none ${i === activeTier ? "text-foreground" : "text-t3"}`}>
                      {tierItem.price}
                    </span>
                  </button>
                ))}
