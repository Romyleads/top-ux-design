    ...
    <section ref={sectionRef} className="mb-24">
      ...
      <div className={`text-center mb-12 transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
        <div className="inline-flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-primary/[.06] flex items-center justify-center">
            {IconComponent && <IconComponent className="w-[18px] h-[18px] text-primary" strokeWidth={1.7} />}
          </div>
          <h2 className="text-2xl font-extrabold text-foreground tracking-tight">{blockTitle}</h2>
          <div className="w-7 h-7 rounded-full bg-primary/[.06] flex items-center justify-center">
            <span className="text-[12px] font-bold text-primary">{cards.length}</span>
          </div>
        </div>
      </div>
