      ...
      <div className="min-h-[88px] px-6 pt-5 pb-3 flex flex-col justify-center">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Link to={`/${locale}/services/${service.id}`} className="min-w-0 flex-1">
            <h3
              className="font-card-title text-[17px] text-foreground leading-[1.15] overflow-hidden card-title-bold-on-hover"
              style={{
                display: '-webkit-box',
                WebkitLineClamp: 2,
                ...
                transition: 'text-shadow 500ms cubic-bezier(0.16, 0.84, 0.24, 1), letter-spacing 500ms cubic-bezier(0.16, 0.84, 0.24, 1)',
              }}
            >
              {serviceName}
            </h3>
              ...
              .map((section, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col gap-1">
                    <span className="text-[11px] font-bold tracking-[0.1em] uppercase" style={{ color: 'hsl(0 84% 50%)' }}>{sectionLabel}</span>
                    {isContentList && contentItems ? (
                      <ul className="list-none p-0 m-0 flex flex-col gap-1">
                        {contentItems.map((item, j) => (
                          <li key={j} className="text-[13px] text-t2 leading-relaxed pl-3 relative before:content-['›'] before:absolute before:left-0 before:text-primary before:font-bold">
                            {item}
                          </li>
                        ))}
                      </ul>
                    ) : (
                      <span className="text-[13.5px] text-t2 leading-relaxed">{sectionText}</span>
                    )}
                  </div>
                );
              })}
